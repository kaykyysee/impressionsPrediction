...
if st.button("Prediksi"):
    if user_text.strip():
        ...
        # TF-IDF transform
        text_vector = vectorizer(tf.convert_to_tensor([processed_text])).numpy()
        
        # Encode domain
        encoded_domain = np.array([[domain_mapping[domain]]])
        
        # Retweets array
        retweets_array = np.array([[retweets]])
        
        # Text length array
        text_length_array = np.array([[len(processed_text.split())]])
        
        # Combine features
        input_features = np.hstack([text_vector, encoded_domain, retweets_array, text_length_array])
